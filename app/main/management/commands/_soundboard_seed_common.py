"""
Base commune pour les commandes de fixture SoundBoard public
(seed_public_soundboard / seed_E2E_public_soundboard).

Le prefixe "_" empeche Django de reconnaitre ce module comme une commande.
"""
...
class BaseSoundboardSeedCommand(BaseCommand):
    """
    Factorise la creation idempotente d'un SoundBoard public de demonstration
    avec playlists (variantes maxDelay) et playlists boutons URL.

    Les sous-classes doivent definir FIXTURE_USER, FIXTURE_SOUNDBOARD_NAME,
    PLAYLIST_BLUEPRINTS et help, et peuvent surcharger
    `_attach_audio_files_for_playlist` pour changer la strategie d'alimentation
    des musiques.
    """

    FIXTURE_USER: dict = {}
    FIXTURE_SOUNDBOARD_NAME: str = ""
    PLAYLIST_BLUEPRINTS: tuple = ()
    ...
    def _ensure_fixture_user(self):
        fixture_user = self.FIXTURE_USER
    ...
    def _ensure_public_fixture_soundboard(self, user):
        soundboard, soundboard_created = SoundBoard.objects.get_or_create(
            user=user,
            name=self.FIXTURE_SOUNDBOARD_NAME,
            defaults={
                "is_public": True,
                "color": "#0f172a",
                "colorText": "#ffffff",
    ...
    def _iter_playlist_payloads(self, max_delay_variants: Iterable[int]) -> Iterable[dict]:
        for blueprint in self.PLAYLIST_BLUEPRINTS:
            for max_delay in max_delay_variants:
                ...
                if "audio_file_name" in blueprint:
