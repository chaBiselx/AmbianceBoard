...
class SharedSoundboardConsummers(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            elif message_type == 'reset_volume_and_hidden_board_playlist':
                await self.handle_reset_volume_and_hidden_board_playlist(data)
    ...
    async def handle_reset_volume_and_hidden_board_playlist(self, data):
    ...
    async def reset_volume_and_hidden_board_playlist(self, event):
        """Reçoit les messages reset_volume_and_hidden_board_playlist du groupe"""
        # Ne pas renvoyer le message à l'expéditeur
        if event.get('sender') != self.channel_name:
            await self.send(text_data=json.dumps({
                'type': 'reset_volume_and_hidden_board_playlist',
